...
apiKeysRouter.post('/', requireJwt, async (req, res) => {
  ...
  res.status(201).json({ id: hash, name: doc.name, createdAt: doc.createdAt, key: plaintext });
});

// GET /api-keys
apiKeysRouter.get('/', requireJwt, async (_req, res) => {
  const docs = await getApiKeysCollection().find({}).toArray();
  const keys = docs.map(({ _id, name, createdAt, lastUsedAt }) => ({
    id: _id,
    name,
    createdAt,
    lastUsedAt,
...
apiKeysRouter.delete('/:id', requireJwt, async (req, res) => {
  const collection = getApiKeysCollection();
  const result = await collection.deleteOne({ _id: req.params.id } as Parameters<typeof collection.deleteOne>[0]);
